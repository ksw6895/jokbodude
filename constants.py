...
점수 부여 기준 (100점 만점, 5점 단위로만 부여) - ⚠️ 극히 엄격한 기준 적용:

**90-100점 (핵심 출제 슬라이드 - 극히 드물게 부여)**:
- 100점: 슬라이드의 텍스트/그림/도표가 문제에 **완전히 동일하게** 출제됨 (복사 수준)
- 95점: 슬라이드의 핵심 내용 90% 이상이 문제에 직접 활용됨
- 90점: 이 슬라이드 하나만으로 문제의 정답과 오답 이유를 모두 설명 가능

**70-85점 (높은 연관성 - 엄격한 기준)**:
- 85점: 문제의 핵심 개념이 슬라이드에 명확히 설명되어 있음
- 80점: 문제 해결에 필요한 주요 정보의 70% 이상 포함
- 75점: 문제와 직접 연관된 내용이 상당 부분 있음
- 70점: 문제 이해와 해결에 중요한 도움이 됨

**50-65점 (중간 연관성 - 신중하게 부여)**:
- 65점: 문제의 배경 지식이 설명되어 있음
- 60점: 문제와 같은 주제를 다루지만 깊이가 다름
- 55점: 부분적으로 도움이 되는 정보 포함
- 50점: 최소한의 관련성 (이 미만은 제외)

**25-45점 (낮은 연관성 - 대부분 이 범위에 해당)**:
- 45점: 같은 단원이지만 직접적 연관성 부족
- 40점: 포괄적 주제는 같으나 세부 내용 다름
- 35점: 약간의 연관성만 존재
- 30점: 매우 간접적인 연관성
- 25점: 최소한의 주제 연관성
...
⚠️ **중요**: 
1. 반드시 5점 단위로만 점수를 부여하세요
2. 90점 이상은 정말 확실한 경우에만 부여
3. 대부분의 슬라이드는 50-70점 범위에 해당
4. 50점 미만은 관련성이 매우 낮은 경우만 부여"""
...
{{
    "related_slides": [
        {{
            ...
            "importance_score": 5-100,  // 5점 단위로만
...
4. 각 강의 슬라이드와의 관련성에 대해 relevance_score를 부여하세요:
   - 반드시 5점 단위로 점수 부여 (5, 10, 15, ..., 90, 95, 100)
   - 90점 이상: 핵심 출제 슬라이드
   - 70-85점: 높은 연관성
   - 50-65점: 중간 연관성
   - 25-45점: 낮은 연관성
                ...
                {{
                    ...
                    "related_lesson_slides": [
                        {{
                            ...
                            "relevance_reason": "관련성 이유",
                            "relevance_score": 5-100  // 5점 단위로만 (5, 10, 15, ..., 95, 100)
                        }}
                    ]
                }}
            ]
