...
- 모든 응답 JSON에는 `jokbo_page`와 `question_number`를 **정확히 기입**하고,
  모델이 판단한 페이지/번호가 틀릴 경우 즉시 경고 메시지를 남기세요.
...
- JSON 문법을 정확히 지켜주세요
- 모든 응답에서 `jokbo_page`, `question_number`, `lesson_page`(필요 시),
  `answer`, `explanation`이 누락되지 않도록 꼭 확인하세요.
...
- 문제와 내용이 섞인 슬라이드의 경우 문제 영역은 무시하고 내용 부분만 고려하며, 70점 미만이면 제외
...
1) 다음 범위의 족보 문제(예: 1~20번)만 분석하세요.
2) 각 문제에 대해 아래 필드를 반드시 충족하세요:
