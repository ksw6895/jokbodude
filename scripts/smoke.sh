...
# Basic end-to-end smoke test using curl + stdlib poller.
# Requires API running (uvicorn) and a Celery worker connected to Redis.
...
echo "[2/3] Submitting jokbo-centric job to ${BASE_URL} ..."
RESP_JSON=$(curl -sS -f \
  -F "jokbo_files=@jokbo/sample.pdf" \
  -F "lesson_files=@lesson/sample.pdf" \
  "${BASE_URL}/analyze/jokbo-centric?model=flash")

echo "Response: ${RESP_JSON}"

# Extract job_id and task_id using Python (no jq dependency)
JOB_ID=$(python - <<'PY'
import json,sys
