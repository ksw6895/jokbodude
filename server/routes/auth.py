...
from ..core import PRO_MODEL_PASSWORD
...
def _get_current_user(session: Optional[str] = Cookie(None)) -> Optional[dict]:
    if not session:
        return None
...
@router.get("/me")
def me(request: Request, user=Depends(_get_current_user)):
    if not user:
